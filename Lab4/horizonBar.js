d3.csv("https://tungth.github.io/data/vn-provinces-data.csv").then((data) => {
  console.log(data);
...
function draw() {
  ...
  y = d3.scaleBand().range([0, h]).padding(0.1);
  yAxis = svg.append("g");

  rect = svg.append("g");
  text = svg.append("g");

  d3.selectAll("button#add").on("click", function () {
    size++;
  ...
  d3.selectAll("button#remove").on("click", function () {
    size--;
    ...
    data = data.sort(function (a, b) {
      switch (criterion) {
        case "name":
          ...
          else if (a["province"] > b["province"]) return 1;
          else return 0;
        case "GDP":
...
function update() {
  ...
  var color = d3
    .scaleSequential()
    .interpolator(d3.interpolateRgbBasis(["yellow", "orange", "red"]))
  ...
  rect
    ...
    }) // highlight start of transition
    .attr("width", (d) => x(d["GRDP-VND"]))
    .on("end", function () {
      d3.select(this).attr("fill", (d) => color(d["GRDP-VND"]));
    }); // revert color at end of transition
  ...
  yAxis
    ...
    .call(
      d3
        .axisLeft(y)
        .tickFormat((d, i) => parseFloat(data[i]["GRDP-VND"]).toFixed(2))
    );
}
